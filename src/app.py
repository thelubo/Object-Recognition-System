...
with tab4:
    ...
    # Взимаме историята на потребителя (може да ограничиш limit или да оставиш None)
    user_id = st.session_state.user_id
    all_docs = get_detection_history(user_id=user_id, limit=500)  # limit=500 за performance

    if not all_docs:
        st.warning("No detection history found for this user.")
    else:
        # Събираме обобщени статистики
        total_objects = 0
        class_counts = {}
        confidences_by_class = {}
